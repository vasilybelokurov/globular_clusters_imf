...
\begin{table*}
    ...
    \begin{tabular}{lcccccc}
        ...
        \hline
        \input{tables/gg23_imf_comparison.tex}
    \end{tabular}
\end{table*}
...
We repeat the same labelled decomposition for the five GG23 disruption calibrations in Table~\ref{tab:gg23_bk_two_component}. A full exact profiled MCMC run for each GG23 two-component model would require refitting two radial profiles and the detectability law at every proposed point in a model-specific GG23 mass coordinate. Instead, for each GG23 calibration we use 60 posterior draws from the corresponding single-component run in Section~\ref{sec:gg23_masses}. At each draw we recompute the GG23 initial masses and survivability surface, re-optimize the two labelled radial profiles and the detectability correction, and then importance-weight the draw by the labelled two-component likelihood relative to the single-component proposal. The last column gives the resulting effective sample size, so rows with concentrated importance weights can be identified directly.

The conclusion is stable across disruption calibrations. The accreted-labelled component remains a minority of the inferred birth population, with $f_{\rm acc}\simeq 0.08$--0.16. The absolute accreted count changes because the total lower-bound population changes under the GG23 mass-loss prescriptions, but most of the inferred disrupted population remains associated with the in-situ-labelled radial component. The GG23 BHs + [Fe/H] row has the lowest effective sample size and should be treated as less numerically stable than the other rows; it nevertheless gives the same qualitative result.

\begin{table*}
    \centering
    \caption{Labelled Belokurov--Kravtsov two-component diagnostic repeated for the GG23 disruption calibrations. The single-component column gives the posterior median from the corresponding GG23 single-component run. The two-component columns are obtained by re-profiling the labelled two-component radial profiles and detectability correction at 60 posterior draws from that single-component run, with importance weights from the labelled likelihood. The final column gives the effective sample size of the importance weights.}
    \label{tab:gg23_bk_two_component}
    \scriptsize
    \begin{tabular}{lccccccc}
        \hline
        Model & Single $N_0$ & Two-comp. $N_0$ & In situ $N_0$ & Accreted $N_0$ & $f_{\rm acc}$ & $\Delta\mathrm{BIC}_{\rm cond}$ & $N_{\rm eff}$ \\
        \hline
        \input{tables/gg23_bk_two_component_summary.tex}
    \end{tabular}
\end{table*}
...
\subsection{Selection-corrected GC properties}\label{sec:discussion_corrected_properties}
The same selection model can be used to ask how other observed GC properties would look after correcting for the missing progenitors implied by the fitted birth model. For an observed cluster $i$ with measured property $z_i$ and intrinsic coordinates $(M_{{\rm ini},i},a_i)$, each posterior selection-surface draw gives the selected fraction
\begin{equation}
p_{{\rm sel},i}^{(k)}
=
S^{(k)}(M_{{\rm ini},i},a_i)\,
Q^{(k)}(M_{{\rm ini},i},a_i).
\end{equation}
We therefore assign that cluster an inverse-selection birth weight
\begin{equation}
w_{{\rm birth},i}^{(k)} = \left[p_{{\rm sel},i}^{(k)}\right]^{-1},
\end{equation}
and, when plotting the implied destroyed contribution on the same observed support, a missing-cluster weight $w_{{\rm dest},i}^{(k)}=w_{{\rm birth},i}^{(k)}-1$. This is not a model for clusters with no surviving analogue in the catalogue. It is the selection-corrected distribution of the progenitors represented by the observed sample, under the same survivability and detectability filters used in the GCIMF inference.

Figure~\ref{fig:detection_corrected_mdf} applies this reweighting to the catalogue metallicities. The observed MDF uses the 164 clusters with finite local metallicities. The birth-corrected MDF has posterior median normalization $465$, with a central 68 per cent range $354$--$647$, while the corresponding missing contribution has median $301$. The correction strengthens the metal-rich component relative to the raw survivor MDF, consistent with the fact that lower-mass clusters on smaller orbits have lower selected fractions.

\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{figures/detection_corrected_metallicity_distribution_kde.pdf}
    \caption{Selection-corrected metallicity distribution. The black curve shows the observed survivor MDF for the 164 clusters with finite catalogue metallicities. The blue curve shows the birth-corrected MDF obtained by weighting each cluster by $w_{\rm birth}=1/[S(M_{\rm ini},a)Q(M_{\rm ini},a)]$. The orange curve shows the corresponding missing contribution, $w_{\rm dest}=w_{\rm birth}-1$, on the same observed support. Shaded bands show the central 68 per cent range over posterior selection-surface samples.}
    \label{fig:detection_corrected_mdf}
\end{figure*}

Figure~\ref{fig:detection_corrected_age_distribution} repeats the exercise for the 55 clusters with ages from \citet{VandenBerg2013}. Because this age sample is incomplete and was not selected by the same process as the full Baumgardt catalogue, the result should be read as a selection-corrected diagnostic of the VandenBerg subsample, not as a complete Galactic GC age distribution. Within that sample, the posterior median birth-corrected total is $115$, with a central 68 per cent range $94$--$148$, and the median missing contribution is $60$. The correction gives more weight to the younger, relatively metal-rich part of the sample, but it does not erase the dominant old population.

\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{figures/detection_corrected_age_distribution_kde.pdf}
    \caption{Selection-corrected age distribution for the 55 clusters with VandenBerg et al. ages. The curves and shaded bands have the same meaning as in Fig.~\ref{fig:detection_corrected_mdf}. The correction is evaluated with the same posterior $S(M_{\rm ini},a)Q(M_{\rm ini},a)$ surfaces used for the MDF, but only at the locations of the clusters with published ages.}
    \label{fig:detection_corrected_age_distribution}
\end{figure*}

The age--metallicity plane gives a more direct view of how the correction acts on the two assembly channels identified by the Belokurov--Kravtsov labels. Figure~\ref{fig:detection_corrected_age_metallicity_origin} shows the VandenBerg age--metallicity sample split into in-situ and accreted classes. The observed subsample contains 30 in-situ and 25 accreted clusters; after median inverse-selection weighting, these become approximately 82 and 33 objects, respectively. The correction therefore increases the relative importance of the in-situ-labelled part of the age--metallicity plane in this subsample. This is the same behaviour found in the labelled two-component birth model: the accreted component is visible and chemically distinct, but most of the selection-corrected progenitor count remains associated with the in-situ-labelled population.

\begin{figure*}
    \centering
    \includegraphics[width=\textwidth]{figures/detection_corrected_age_metallicity_by_origin.pdf}
    \caption{Origin-split age--metallicity density for the 55 clusters with VandenBerg et al. ages and metallicities. Blue denotes in-situ-labelled clusters and orange denotes accreted-labelled clusters. Left: observed survivor sample. Right: birth-corrected sample, with point sizes scaled by the median inverse-selection weight. Contours show the 5, 10, 20, 40, and 60 per cent enclosed KDE density levels, computed separately for each origin class and panel; they therefore compare the shape of each distribution rather than absolute density between panels.}
    \label{fig:detection_corrected_age_metallicity_origin}
\end{figure*}
