...
These results motivate the approach taken here. We distinguish the intrinsic GC birth function from the probability that a cluster of given initial mass and orbit survives to the present day and from the probability that a surviving cluster is detected in the observed catalogue. This separation is essential because the Milky Way data constrain neither a pure formation-scale $M_{\rm c}$ nor a pure dynamical mass-loss scale in isolation. Instead, they constrain the combination of the Schechter cutoff, the radial birth profile, the survivability boundary in the $(M_{\rm ini},a)$ plane, and the observational selection function. The open questions addressed by this paper are therefore: what range of $M_{\rm c}$ is allowed by the surviving Galactic GC catalogue once survivability is modelled explicitly; how degenerate this cutoff is with the assumed lifetime scale and radial birth profile; how many clusters above $10^4\,{\rm M}_\odot$ are required to have formed in order to explain the surviving system; and how much of the missing population is expected to lie at small semimajor axis, where debris is likely to be dynamically mixed rather than visible as cold tidal streams.
...
In this paper we infer the Milky Way GCIMF by modelling the Baumgardt GC catalogue in the plane of inferred initial mass and orbital semimajor axis \citep{Baumgardt2019,BaumgardtCatalog2023}. We write the intrinsic GC birth intensity as a universal IMF multiplied by a radial birth profile, then filter it by a dynamical survivability surface and an observational detectability correction. The result is a forward model for the surviving catalogue and, at the same time, an estimate of the missing disrupted population that should seed stellar streams, phase-mixed inner-Galaxy debris, and compact-object products.
